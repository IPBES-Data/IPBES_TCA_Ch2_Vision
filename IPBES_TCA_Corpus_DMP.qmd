...
if (!file.exists(fn)) {
    actors_overlap_cases <- file.path("tca_corpus", "data", "actors_ids_cases.rds") |>
        readRDS()

    actors_overlap_cases <- sapply(
        names(actors_overlap_cases),
        FUN = function(s1) {
            sapply(
                names(actors_overlap_cases),
                function(s2) {
                    length(intersect(actors_overlap_cases[[s1]], actors_overlap_cases[[s2]]))
                }
            )
        }
    )

    saveRDS(actors_overlap_cases, file = fn)
    write.csv(actors_overlap_cases, file = gsub("rds$", "csv", fn))
}
```
