...
if (n < 12) {
    ...
    p <- as.data.frame(as.table(data)) |>
        dplyr::rename("Strategies / Optinons" = Var1, "Actors" = Var2, "Proportion" = Freq) |>
        ggplot(
            aes(x = Actors, y = `Strategies / Optinons`, fill = Proportion)
        ) +
        geom_tile() +
        scale_fill_gradient2(
            low = "#2166ac", # Blue color for negative values
            mid = "#f7f7f7", # Light beige color for zero
            high = "#b2182b", # Red color for positive values
...
if (n < 4) {
    ...
    # data$hline <- c(0, diff(as.numeric(factor(sector))) != 0)
    # data$vline <- c(0, diff(as.numeric(factor(strategy))) != 0)

    p <- as.data.frame(as.table(data)) |>
        dplyr::rename("Strategies / Optinons" = Var1, "Actors" = Var2, "Difference in Proportion" = Freq) |>
        ggplot(
            aes(x = Actors, y = `Strategies / Optinons`, fill = `Difference in Proportion`)
        ) +
        geom_tile() +
        scale_fill_gradient2(
            low = "#2166ac", # Blue color for negative values
