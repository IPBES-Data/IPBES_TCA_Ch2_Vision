...
```{r}
#| label: get_art_media_hits
#|

fn <- file.path(".", "data", "tca_corpus", "art_media_table.rds")
if (!file.exists(fn)) {

    st <- paste0("(", params$s_1_tca_corpus, ") AND (", params$st_art, ")") |>
        compact()

    c_art <- openalexR::oa_fetch(title_and_abstract.search = st, count_only = TRUE, verbose = TRUE, output = "list")$count

    st <- paste0("(", params$s_1_tca_corpus, ") AND (", params$st_media, ")") |>
        compact()

    c_media <- openalexR::oa_fetch(title_and_abstract.search = st, count_only = TRUE, verbose = TRUE, output = "list")$count

    st <- params$s_1_tca_corpus |>
        compact()

    c_all <- openalexR::oa_fetch(title_and_abstract.search = st, count_only = TRUE, verbose = TRUE, output = "list")$count

    data.frame(
        Category = c("TCA Corpus", "Media Subset", "Art Subset"),
        Count = c(c_all, c_media, c_art),
        Proportion = c(c_all, c_media, c_art) / c_all,
        Timestamp = Sys.time()
    ) |>
    saveRDS(file = fn)
}
...
## Art and Media subsets

```{r}
file.path(".", "data", "tca_corpus", "art_media_table.rds") |>
    readRDS() |>
    knitr::kable()
```
...
This data is corrected for ifferent research oputput before and after 1992 by dividing by the overall reasarch output in thap perio as reflected on [OpenAlex](https://openalex.org).
...
![The graph shows the by overall research output corrected publications before and after 1992 for each `Strategy ||| Concept`](`r file.path("figures", "tca_corpus", "strategies_options_time_split.png")`)

To download high resolution, [click here](`r file.path("figures", "tca_corpus", "strategies_options_time_split.pdf")`){target="_blank"}

![The second graph shows the proportion of the by overall research output corrected publications before and after 1992 for each `Strategy ||| Concept`](`r file.path("figures", "tca_corpus", "strategies_options_time_split_p.png")`)
