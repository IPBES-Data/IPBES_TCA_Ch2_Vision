...
fn <- file.path("data", "tca_corpus", "random_50_tca_corpus.xlsx")
if (!file.exists(fn)) {
    set.seed(13)
...
if (!file.exists(fn)) {
    ...
    IPBES.R::corpus_read(sector_dir) |>
        arrow::write_parquet(
            paste0(sector_dir, ".parquet2")
        )
    unlink(sector_dir, recursive = TRUE)
}
```
...
if (!file.exists(fn)){
    data <- IPBES.R::corpus_read(file.path(".", "data", "tca_corpus", "sectors.parquet")) |>
        dplyr::group_by(
            publication_year,
            sector,
            i
