...
if (!file.exists(paste0(fn, ".html"))) {
    ...
    nodes$x <- rep(1:grid_size, each = grid_size)[1:nrow(nodes)] * 200 # scale x coordinates
    nodes$y <- rep(1:grid_size, times = grid_size)[1:nrow(nodes)] * 200 # scale y coordinates
...
fn <- file.path("figures", "tca_corpus", "dendrogram_interactive.html")
if (!file.exists(fn)) {
    ...
    unlink(file.path("figures", "tca_corpus", "dendrogram_interactive_files"), recursive = TRUE)
}
```

<iframe src="figures/tca_corpus/dendrogram_interactive.html" width="100%" height="800px"></iframe>

To open the interactive graph or to download it, [click here](figures/tca_corpus/dendrogram_interactive.html){target="_blank"}
...
if (length(list.files(file.path("figures", "tca_corpus"), pattern = "dendrogram_circular")) < 2) {
    ...
    ggplot2::ggsave(
        plot = p,
        file.path("figures", "tca_corpus", "dendrogram_circular.pdf"),
        width = 12,
        height = 6
    )
    ggplot2::ggsave(
        plot = p,
        file.path("figures", "tca_corpus", "dendrogram_circular.png"),
        width = 12,
        height = 6
    )
}
...
![](`r file.path("figures", "tca_corpus", "dendrogram_circular.png")`)

To download high resolution, [click here](figures/tca_corpus/dendrogram_circular.pdf`){target="_blank"}
