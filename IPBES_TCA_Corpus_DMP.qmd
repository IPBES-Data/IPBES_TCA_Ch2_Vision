...
pages_dir <- file.path(".", "data", "tca_corpus", "pages")
arrow_dir <- file.path(".", "data", "tca_corpus", "corpus")
...
if (!file.exists(fn)) {
    data <- file.path(".", "data", "tca_corpus", "corpus") |>
        IPBES.R::arrow_read_corpus() |>
        dplyr::select(author) |>
        dplyr::collect()

    countries_tca_corpus <- pbmcapply::pbmclapply(
        1:nrow(data),
        function(i) {
            country <- IPBES.R::unserialize_arrow(data$author[i])[[1]]["institution_country_code"] |>
                unlist()
            weight <- 1 / length(country)
            result <- data.frame(
                country = country,
                weight = weight
            )
...
```{r}
#| label: get_countries_tca_corpus_b2016
#|

fn <- file.path(".", "data", "tca_corpus", "countries_tca_corpus_b2016.rds")
if (!file.exists(fn)) {
    data <- file.path(".", "data", "tca_corpus", "corpus") |>
        IPBES.R::arrow_read_corpus() |>
        dplyr::filter(publication_year <= 2016) |>
        dplyr::select(author) |>
        dplyr::select(author) |>
        dplyr::collect()

    countries_tca_corpus_b2016 <- pbmcapply::pbmclapply(
        1:nrow(data),
        function(i) {
            country <- IPBES.R::unserialize_arrow(data$author[i])[[1]]["institution_country_code"] |>
                unlist()
    ...
    saveRDS(countries_tca_corpus_b2016, file = fn)
} else {
    countries_tca_corpus_b2016 <- readRDS(fn)
}
```

```{r}
#| label: get_countries_tca_corpus_a2018
#|

fn <- file.path(".", "data", "tca_corpus", "countries_tca_corpus_a2018.rds")
if (!file.exists(fn)) {
    data <- file.path(".", "data", "tca_corpus", "corpus") |>
        IPBES.R::arrow_read_corpus() |>
        dplyr::filter(publication_year > 2018) |>
        dplyr::select(author) |>
        dplyr::select(author) |>
        dplyr::collect()

    countries_tca_corpus_a2018 <- pbmcapply::pbmclapply(
        1:nrow(data),
        function(i) {
            country <- IPBES.R::unserialize_arrow(data$author[i])[[1]]["institution_country_code"] |>
                unlist()
    ...
    saveRDS(countries_tca_corpus_a2018, file = fn)
} else {
    countries_tca_corpus_a2018 <- readRDS(fn)
}
```
...
data <- read_corpus(file.path("data", "tca_corpus", "corpus")) |>
