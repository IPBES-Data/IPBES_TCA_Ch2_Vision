...
build <- as.integer(readLines("buildNo_ch2_evidence_causes"))
build <- build + 1
writeLines(as.character(build), "buildNo_ch2_evidence_causes")
...
## TODO Download `technology AND vision` Corpus

The corpus download will be stored in `ch1_evidence_causes/pages` and the arrow database in `data/ch1_evidence_causes/corpus`.
...
### TODO Convert TCA Corpus to Arrow
...
### TODO Extract 50 random papers from `technology AND vision` in TCA Corpus
```{r}
...
The Narrowed TFC Corpus consists of **`r count$narrowed_tca`** works, which is **`r round(100 * count$narrowed_tca_tca / count$tca_corpus, digits = 2)`%** of the TCA Corpus.
...
This is not possible due to the length of the query. A download of the Narrowed TFC Corpus is suggested, but will take time due to its size.

### 6. Assessing the literature that discusses the causes of biodiversity loss 

#### Nature Corpus

There are **`r count$biodiv_loss_nat_s_1`** works in the Nature Corpus that discuss the causes of biodiversity loss. 

#### Narrowed TFC Corpus

There are **`r count$biodiv_loss_narrowed_s_2`** works in the Narrowed TFC Corpus that discuss the causes of biodiversity loss. 

#### Intersection of Nature Corpus and Narrowed TCA Corpus

There are **`r count$biodiv_loss_nat_narrowed_s_3`** works in the intersection from the Nature Corpus and the Narrowed TFC Corpus that discuss the causes of biodiversity loss. 
