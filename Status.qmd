...
params:
    ...
    s_tfc_tsu: "('transformative change' OR 'deliberate transformation*' OR 'transformative turn*' OR 'transition*' OR 'social-ecological change*' OR 'deep change' OR 'fundamental alteration' OR 'profound change' OR 'profound transformation' OR 'radical transformation' OR 'transformational change' OR 'complete change' OR 'complete transformation' OR 'drastic change' OR 'in-depth transformation' OR 'progressive change' OR 'radical alteration' OR 'radical change' OR 'revolutionary change' OR 'significant modification' OR 'total transformation' OR 'transition' OR 'pathway' OR 'power' OR 'agency' OR 'scal*' OR 'lever*' OR 'context' OR 'process' OR 'regime' OR 'shift' OR 'views' OR 'value*' OR 'structure*' OR 'institution*' OR 'deliberate' OR 'structural' OR 'fundamental' OR 'system*' OR 'deep' OR 'radical' OR 'profound' OR 'drastic' OR 'widespread' OR 'politic*' OR 'economic*' OR 'structur*' OR 'complete' OR 'progressive' OR 'revolutionary' OR 'substanti*' OR 'significan*' OR 'radical shift', 'transform shift' OR 'transform turn)"
...
key_papers <- list(
    Hannah = read.csv("key paper Hannah.csv")[, c("DOI", "Title")] |>
        mutate(source = "Hannah"),
    Sebastian = read.csv("key paper Sebastian.csv")[, c("DOI", "Title")] |>
        mutate(source = "Sebastian"),
    Victoria = read.csv("key paper Victoria.csv")[, c("DOI", "Title")] |>
        mutate(source = "Victoria")
)    
```

```{r}
#| label: hits_key_papers
#| 
# TODO TO CHECK

fn <- file.path(".", "data", "hits_key_papers.rds")
if (!file.exists(fn)) {
  hits_key_papers <- lapply(
    key_papers,
    function(kp) {
      dois <- key_papers |>
        select(DOI) |>
      ...
      hits_keypapers <- cbind(
        ...
        R1_tf = get_count(search_term = params$s_tfc, dois = dois),
        R1_tf_Ch5 = get_count(search_term = params$s_tfc_5, dois = dois),
  ...
  saveRDS(hits_key_papers, file = fn)

  unlink(file.path(".", "data", "key_works.rds"))
  unlink(file.path(".", "data", "key_works_df.rds"))
} else {
  key_paper <- readRDS(file = fn)
}
```
...
if (!file.exists(fn)) {
  lapply(
    key_papers,
    function(kp) {
      key_works <- openalexR::oa_query(doi = kp$DOI) |>
        openalexR::oa_request(count_only = FALSE)
    }
  )
  saveRDS(key_works, file = fn)
  ...
  saveRDS(key_works_df, file.path(".", "data", "key_works_df.rds"))
} else {
  key_works <- readRDS(file = fn)
  key_works_df <- readRDS(file.path(".", "data", "key_works_df.rds"))
}
```
...
if (!file.exists(fn)) {
  key_concepts <- lapply(
    ...
    function(kwdf) {
      sapply(
        key_works_df$concepts,
        function(x) {
          # x$display_name[x$score > 0.6]
          x$id[x$score > params$concept_cuttoff]
        }
      ) |>
        unlist() |>
        unique() |>
  ...
  saveRDS(key_concepts, file = fn)
} else {
  key_concepts <- readRDS(file = fn)
}
...
hits_keypapers <- readRDS(file.path(".", "data", "hits_key_papers.rds"))
...
knitr::kable(
    caption = 'Key Paper Sebastian Villasante (two without DOI - not included here!)'
)
```
...
# New search strategy
```{r}
#| lebel: new_search_strategy_test
#| eval: false

openalexR::oa_query(search = params$s_tfc_tsu) |>
...
f1 <- function(dois = NULL) {
    openalexR::oa_query(
        search = params$s_tfc_tsu,
        filter = list(concepts.id = concepts), 
        doi = dois
    ) |>
  openalexR::oa_request(count_only = TRUE) |>
  unlist()
