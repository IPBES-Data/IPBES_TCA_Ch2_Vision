...
if (file.exists(fn)) {
    ...
    hits_keypapers <- cbind(
        R1_transform     = get_count(search_term = params$s_tfc, dois = key_papers),
        R1_transform_Ch5 = get_count(search_term = params$s_tfc_5, dois = key_papers),
        R1_nature        = get_count(search_term = params$s_nat, dois = key_papers),
        R1_nature_Ch5    = get_count(search_term = params$s_nat_5, dois = key_papers),
        R1_nature_VA     = get_count(search_term = params$s_nat_va_new, dois = key_papers),
...
hits_keypapers |>
    apply(c(1, 2), function(x) {
        ifelse(x == 0, "<font color='green'>0</font>", paste0("<font color='red'>", x, "</font>"))
    }
) |>
knitr::kable(
